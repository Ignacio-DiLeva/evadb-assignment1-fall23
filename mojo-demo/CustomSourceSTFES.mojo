...
struct CustomSentenceTransformerFeatureExtractor1:
    var ready: Bool
    var model: PythonObject

    fn manifest(self) raises -> PythonObject:
        return FunctionManifest()
            .name("CustomSentenceTransformerFeatureExtractor1")
            .cacheable(False)
...
struct CustomSentenceTransformerFeatureExtractor2:
    var ready: Bool
    var model: PythonObject

    fn manifest(self) raises -> PythonObject:
        return FunctionManifest()
            .name("CustomSentenceTransformerFeatureExtractor2")
            .cacheable(False)
            .function_type("mojo/FeatureExtraction")
            .batchable(False)
            .input_signatures(mlist(
...
var stfe1 = CustomSentenceTransformerFeatureExtractor1()
var stfe2 = CustomSentenceTransformerFeatureExtractor2()
...
fn do_manifests() raises:
    _ = mojoh.pickle_store(FILES_FOLDER + "/manifests/CustomSentenceTransformerFeatureExtractor1", stfe1.manifest())
    _ = mojoh.pickle_store(FILES_FOLDER + "/manifests/CustomSentenceTransformerFeatureExtractor2", stfe2.manifest())

fn do_exec(cmd: PythonObject) raises:
    if cmd == "setup:CustomSentenceTransformerFeatureExtractor1":
        stfe1.setup()
        return
    elif cmd == "setup:CustomSentenceTransformerFeatureExtractor2":
        stfe2.setup()
        return
    elif cmd == "forward:CustomSentenceTransformerFeatureExtractor1":
        let df = mojoh.pickle_load(FILES_FOLDER + "/infile")
        _ = mojoh.pickle_store(FILES_FOLDER + "/outfile", stfe1.forward(df))
    elif cmd == "forward:CustomSentenceTransformerFeatureExtractor2":
        let df = mojoh.pickle_load(FILES_FOLDER + "/infile")
        _ = mojoh.pickle_store(FILES_FOLDER + "/outfile", stfe2.forward(df))
    else:
